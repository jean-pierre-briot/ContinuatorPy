...
# Version 2.1
# Polyphonic, batch or real-time mode, richer state/viewpoint (note/pitch, duration, velocity), transposition, tolerance matching
# Still under work
# 03/03/2025
# Jean-Pierre Briot
...
_default_fixed_duration = 0.1               # Fixed: fixed (_default_fixed_duration) duration
_chord_notes_start_threshold = 0.05
...
def pitch_sequence_from_note_sequence(note_sequence):
    print('pitch_sequence_from_note_sequence: note_sequence: ' + str(note_sequence))
    pitch_sequence = []
    for note in note_sequence:
        pitch_sequence.append(note.pitch)
    print('pitch_sequence_from_note_sequence: ' + str(pitch_sequence))
...
class PrefixTreeNode:                       # Structure of a tree node to memorize and index learnt sequences
    def __init__(self):
        self.note_list = None
        self.pitch_tuple = None
        self.children_list = None
        self.continuation_index_list = None

    def match(self, note_list, tolerance):       # Check if current node characteristics (e.g., its corresponding note pitch) is matching some note characteristics (e.g., that note pitch)
        pitch_tuple = tuple(pitch_sequence_from_note_sequence(note_list))
        return self.pitch_tuple == pitch_tuple
...
class PrefixTreeContinuator:                # The main class and corresponding algorithms
    def __init__(self):
        ...
        self.current_chord_note_sorted_list = []
        self.last_note_end_time = None
        self.played_notes = []
        self.continuation_sequence = []
        self.first_current_note_on_start_time = None

    def train(self, note_list_sequence):         # Main entry function lo train the Continuator with a sequence of notes
                                            # note_sequence = [(<pitch_1>, <duration_1>, <velocity_#), ... , (<pitch_N>, <duration_N>, <velocity_N>)]
        self.internal_train_without_key_transpose(note_list_sequence)    # Train with input sequence
        if _key_transposition_mode:
            for i in range(-5, 0):
                self.internal_train_without_key_transpose(self.transpose(note_list_sequence, i))
            for i in range(1, 7):
                self.internal_train_without_key_transpose(self.transpose(note_list_sequence, i))

    def internal_train_without_key_transpose(self, note_list_sequence):
        self.internal_train_without_any_transpose(note_list_sequence)
        if _octave_transposition_number > 0:
            note_pitch_sequence = []
            for note in note_list_sequence:
            ...
            while i <= _octave_transposition_number and max_pitch + (i * 12) <= 128:
                self.internal_train_without_any_transpose(self.transpose(note_list_sequence, i * 12))
                i += 1
    ...
    def internal_train_without_any_transpose(self, note_list_sequence):  # Main internal train function
        if not self.root_dictionary and len(note_list_sequence) <= 1:
            raise RuntimeError('Only one note initially played, thus no continuation may be learnt and therefore generated')
        i = len(note_list_sequence) - 1                                  # index of the last item of the played note sequence
        while i > 0:
                                                                    # i will vary from length-1 (note_N-1) to 0 (note_1)
            note_list_sub_sequence = note_list_sequence[:i]                   # Prefix Sub-sequence: [note_1, .. , note_i]
            continuation = note_list_sequence[i]                         # Continuation = event_i
            self.continuation_dictionary[self.continuation_dictionary_current_index] = continuation
            reversed_note_list_sub_sequence = note_list_sub_sequence[::-1]    # Reverse Sub-sequence: [note_i, ..., note_1]
            root_note_list = reversed_note_list_sub_sequence[0]               # Last note of the sub sequence note_i is the note to be searched/matched as a root of a tree
            pitch_list = pitch_sequence_from_note_sequence(root_note_list)
            pitch_tuple = tuple(pitch_list)
            if pitch_tuple not in self.root_dictionary:         # If the note list has not yet some corresponding prefix tree root,
                current_node = PrefixTreeNode()                     # then, creation of the corresponding new tree (root)
                self.root_dictionary[pitch_tuple] = current_node
                current_node.note_list = root_note_list
                current_node.pitch_tuple = pitch_tuple
                ...
                current_node = self.root_dictionary[pitch_tuple]
            ...
            for note_list in reversed_note_list_sub_sequence[1:]:             # Iterative traversal for matching ith level node of the reverse input sequence
                ...
                if current_node.children_list is None:              # If there is no children, then, we have met a terminating leaf,
                    ...
                    new_child_node.note_list = note_list
                    new_child_node.pitch_tuple = tuple(pitch_sequence_from_note_sequence(note_list))
                    ...
                    for child_node in current_node.children_list:   # while iterating over the children
                        if child_node.match(note_list, 0):               # This child (exactly) matches
    ...
    def display_tree(self, node, level):
        print('display_tree: node: ' + str(node) + ' node.note_list: ' + str(node.note_list) + ' node.pitch_tuple: ' + str(node.pitch_tuple))
        indent = '  ' * level
        print(indent, end='')
        continuation_pitch_list = []
        for i in range(len(node.continuation_index_list)):
            print(str(i) + ' ' + str(node.continuation_index_list[i]))
            print(str(self.continuation_dictionary[node.continuation_index_list[i]]))
            continuation_pitch_list.append(self.continuation_dictionary[node.continuation_index_list[i]])
        print(str(pitch_sequence_from_note_sequence(node.note_list)))
        print(str(node.pitch_tuple))
        if node.children_list is not None:
            for child in node.children_list:
                self.display_tree(child, level + 1)

    def generate(self, note_list_sequence):                              # Generation of a continuation
        length_note_list_sequence = len(note_list_sequence)                   # Remember length of the played input sequence of notes, because note_sequence will be expanded (append)
        last_input_note_list = note_list_sequence[-1]                         # We start with the last note of the reverse sequence: Note_N
        ...
        for i in range(1, _max_continuation_length):
            ii = i
            last_input_note_list_pitch_tuple = tuple(pitch_sequence_from_note_sequence(last_input_note_list))
            if last_input_note_list_pitch_tuple not in self.root_dictionary:   # If there is no matching tree root thus we cannot generate a continuation
                if _general_default_random_generation_mode:         # If default random generation mode
                    next_note_list = self.continuation_dictionary[random.randint(1, len(self.continuation_dictionary))]
                    note_list_sequence.append(next_note_list)                 # Add this continuation note to the list of input notes
                    self.continuation_sequence.append(next_note_list)    # Add this continuation note to the list of continuations
                    last_input_note_list = next_note_list                     # And continue the generation from this (new) last note
                    ...
                    match _generation_duration_mode:
                        ...
                        case 'Played':
                            if ii > len(note_list_sequence):
                                ii = i - len(note_list_sequence)
                            next_note_list.duration = note_list_sequence[ii - 1].duration
                        case 'Fixed':
                ...
                current_node = self.root_dictionary[pitch_sequence_from_note_sequence(last_input_note_list)]
                ...
                while current_node.children_list is not None and j < length_note_list_sequence:
                    ...
                    for child in current_node.children_list:        # Iterate over children nodes to look for a node matching jth last note from input sequence
                        if child.match(note_list_sequence[-j], _match_pitch_interval_tolerance):
                ...
                if current_node.children_list is None or j >= length_note_list_sequence or matching_child is None:
                    ...
                    next_note_list = self.continuation_dictionary[current_node_continuation_index_list[random.randint(0, len(current_node_continuation_index_list) -1)]]
                    ...
                    match _generation_duration_mode:
                        ...
                        case 'Played':
                            ...
                            next_note_list.duration = note_list_sequence[ii - 1].duration
                        case 'Fixed':
                            next_note_list.duration = _default_fixed_duration
                    note_list_sequence.append(next_note_list)                 # Add this continuation note to the list of input notes
                    self.continuation_sequence.append(next_note_list)    # Add this continuation note to the list of continuations
                    last_input_note_list = next_note_list                     # And continue the generation from this (new) last note
    ...
    @staticmethod
    def play_midi_note_list(port_name, note_list):
    ...
    def add_to_note_list_sorted_by_pitch(self, note, note_list_sorted_by_pitch):
        # print('entry add note: ' + str(note.pitch) + ' to note_list_sorted_by_pitch: ' + str(pitch_sequence_from_note_sequence(note_list_sorted_by_pitch)))
        for i in range(0, len(note_list_sorted_by_pitch)):
            # print('add note: i: ' + str(i))
            note_list_sorted_by_pitch = note_list_sorted_by_pitch.copy()
            if note.pitch == note_list_sorted_by_pitch[i]:
                raise RuntimeError('Cannot insert note ' + str(note) + ' with the same pitch ' + str(note.pitch) + ' already present into chord note list ' + str(note_list_sorted_by_pitch))
            elif note.pitch < (note_list_sorted_by_pitch[i].pitch):
                # print('add note: ' + str(note.pitch) + ' pitch_sequence: ' + str(pitch_sequence_from_note_sequence(note_list_sorted_by_pitch)))
                note_list_sorted_by_pitch.insert(i, note)
                # print('add note after insert: ' + str(note.pitch) + ' pitch_sequence: ' + str(pitch_sequence_from_note_sequence(note_list_sorted_by_pitch)))
                break
            else:   # If note.pitch > self.chord_note_list_sorted_by_pitch[i],
                continue
        # print('end add note: ' + str(note.pitch) + ' new note_list_sorted_by_pitch: ' + str(pitch_sequence_from_note_sequence(note_list_sorted_by_pitch)))
        return note_list_sorted_by_pitch
    ...
    def listen_and_continue(self, input_port, output_port):
        with mido.open_input(input_port) as in_port, mido.open_output(output_port) as dummy:
            ...
            self.chord_note_list_sorted_by_pitch = []
            self.last_note_end_time = time.time()
            while True:                                             # Infinite listening loop
                for event in in_port.iter_pending():
                    # print('event: ' + str(event))
                    if event.type == 'note_on' and event.velocity > 0:  # If a new note has been played (note_on)
                        if event.note in self.current_note_on_dict:
                            print('Warning: Note ' + str(note.pitch) + ' has been repeated before being ended')
                            continue
                        else:
                            self.continuation_sequence = []         # Reset a future new continuation
                            note = Note(event.note, None, event.velocity)  # Create the new note
                            current_time = time.time()
                            print('event.type == note_on and event.velocity <= 0: time.time(): ' + str(current_time))
                            if not self.current_note_on_dict:            # If there is no ongoing note_on,
                                self.current_note_on_dict[note.pitch] = (note, current_time)  # then, add this note to the (empty) dictionary of ongoing note_on
                                self.chord_note_list_sorted_by_pitch = [note]
                                self.first_current_note_on_start_time = current_time    # and remember current time as the start time of a potential chord
                                print('first note: ' + str(note.pitch))
                            elif current_time < self.first_current_note_on_start_time + _chord_notes_start_threshold:   # Otherwise, there is some ongoing note_on
                                                                    # If current note start time is close enough to start time of the first note of the potential chord,
                                self.current_note_on_dict[note.pitch] = (note, current_time)  # then, add this note to the dictionary of ongoing note_on
                                self.chord_note_list_sorted_by_pitch = self.add_to_note_list_sorted_by_pitch(note, self.chord_note_list_sorted_by_pitch)
                                current_note_on_pitch_list = []
                                for tuple_note_duration in self.current_note_on_dict.values():
                                    current_note_on_pitch_list.append(tuple_note_duration[0])
                                print('note_on_dict: ' + str(current_note_on_pitch_list))
                                print('add note: ' + str(note.pitch))
                            else:                                   # Otherwise, this is a new note independent from and afterwards current potential chord,
                                self.played_notes.append(self.chord_note_list_sorted_by_pitch)  # then, consider the potential chord as a played event
                                print('add played_notes: ' + str(pitch_sequence_from_note_sequence(self.chord_note_list_sorted_by_pitch)))
                                self.current_note_on_dict[note.pitch] = (note, current_time)  # add this note to the (empty) dictionary of ongoing note_on
                                self.chord_note_list_sorted_by_pitch = [note]
                    elif ((event.type == 'note_off') or (event.type == 'note_on' and event.velocity == 0)) and (event.note in self.current_note_on_dict):
                        print('(event.type == note_off) or (event.type == note_on and event.velocity == 0)) and (event.note in self.current_note_on_dict): time.time(): ' + str(current_time))
                    ...
                    #    print('elif (event.type == note_off) or (event.type == note_on and event.velocity == 0)')
                    # else:
                    #    print('last case')
                # print('listen_and_continue after test note_on_off')
                silence_duration = time.time() - self.last_note_end_time    # When there is no more played notes pending events
                # print('after loop for event in in_port.iter_pending()')
                if self.continuation_sequence:                      # If still continuation notes to be played,
                    print('play_midi_note_list: ' + str(self.continuation_sequence))
                    self.play_midi_note_list(output_port, self.continuation_sequence.pop(0))     # then, play the first one (and remove it)
                    ...
                    pitches = []
                    for note_list in self.played_notes:
                        pitches.append(pitch_sequence_from_note_sequence(note_list))
                    print('train: played_notes: ' + str(pitches))
