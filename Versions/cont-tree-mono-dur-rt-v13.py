...
class PrefixTreeContinuator:
    ...
    def train(self, sequence):
        """
        Ajoute une séquence à l'arbre préfixé en ne conservant que les sous-séquences à partir du début.
        Ex : ABCD -> [A], [A, B], [A, B, C]
        """
        notes = [note[0] for note in sequence]  
        self.sequences.append(notes)

        # On génère uniquement les sous-séquences qui commencent à A
        for end in range(1, len(notes)):  # On exclut la séquence complète
            current_node = self.root
            for i in range(end):  # On parcourt seulement depuis le début
                note = notes[i]
                if note not in current_node.children:
                    current_node.children[note] = PrefixTreeNode()
                current_node = current_node.children[note]

            # Ajouter la continuation (prochaine note)
