...
class PrefixTreeContinuator:
    ...
    def display_memory(self, node=None, prefix=[], level=0):
        """Affiche une représentation de l?arbre préfixé monophonique avec indentation claire."""
        if node is None:
            node = self.root
            print("\n?? **Arbre préfixé - Mémoire des séquences enregistrées**")
        
        for note, child in node.children.items():
            indent = "    " * level  # Indentation pour la visualisation claire des sous-arbres
            print(f"{indent}{' -> '.join(map(str, prefix + [note]))}  | Continuations: {child.continuations}")
            self.display_memory(child, prefix + [note], level + 1)
