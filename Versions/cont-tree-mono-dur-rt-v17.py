...
class PrefixTreeContinuator:
    ...
    def train(self, sequence):
        """
        Ajoute une séquence à l'arbre préfixé en conservant des arbres distincts.
        Chaque séquence mémorisée a une racine qui correspond à sa dernière note.
        """
        notes = [note[0] for note in sequence]  
        self.sequences.append(notes)

        # Définir la racine de l'arbre correspondant à la dernière note
        root_note = notes[-1]
        if root_note not in self.roots:
            self.roots[root_note] = PrefixTreeNode()
        current_node = self.roots[root_note]

        # Construire l'arbre préfixé spécifique à cette séquence
