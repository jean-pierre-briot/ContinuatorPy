...
_max_continuation_length = 15				# Maximum number of notes of a continuation
_max_played_notes_considered = 15		    # Maximum last number of played notes considered for training
...
def create_note(pitch, duration, velocity): # Structure and creation of a note
    return ((pitch, duration, velocity))    # (<pitch>, <duration>, <velocity) tuple
                                            # This could be alternatively represented by a class

def note_pitch(note):                       # Related access functions
    return(note[0])
...
class PrefixTreeContinuator:                # The main class and corresponding algorithms
    def __init__(self):
        ...
        self.last_note_time = time.time()   # Remembering time start for last note event
    ...
    def internal_train_without_key_transpose(self, note_sequence):
        ...
        if _octave_transposition_mode:      # If octave transposition,
            note_pitch_sequence = list(map(note_pitch, note_sequence))  # then, transpose pitches of all notes of the sequence in octaves within MIDI pitches limits [O, 128]
    ...
    def transpose(self, note_sequence, t):  # Transpose pitches of all notes of the sequence in some offset
