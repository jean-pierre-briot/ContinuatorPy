...
class PrefixTreeContinuator:
    ...
    def train(self, sequence):
        """
        Ajoute une séquence à l'arbre préfixé en fusionnant correctement les sous-arbres.
        ...
        for end in range(1, len(notes)):  
            root_note = notes[end]  # Racine = dernière note de la séquence partielle
            current_node = self.root

            # Fusionner avec les sous-arbres existants
            if root_note not in current_node.children:
                current_node.children[root_note] = PrefixTreeNode()
            current_node = current_node.children[root_note]

            # Construire le reste de l'arbre en partant de la fin
            ...
            if notes[end] not in current_node.continuations:
                current_node.continuations.append(notes[end])
