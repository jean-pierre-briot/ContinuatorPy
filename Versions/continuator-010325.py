...
class Note:                                 # Structure of a note
    def __init__(self):                     # Instance variables: <pitch>, <duration, <velocity>
        self.pitch = None                   # Pitch
        self.duration = None                # Duration
        self.velocity = None                # Velocity
                                            # We must provide initial default values (even if not used)
...
def create_note_sequence(pitch_sequence):
    ...
    for pitch in pitch_sequence:
        note = Note()
        note.pitch = pitch
        note.duration = _default_generated_note_duration
        note.velocity = _default_generated_note_velocity
...
class PrefixTreeContinuator:                # The main class and corresponding algorithms
    ...
    def transpose(note_sequence, t):        # Transpose pitches of all notes of the sequence in some offset
        ...
        for i in range(0, len(note_sequence)):
            note = Note()
            note.pitch = note_sequence[i].pitch + t
            note.duration = note_sequence[i].duration
            note.velocity = note_sequence[i].velocity
            transposed_note_sequence.append(note)
    ...
    @staticmethod
    def play_midi_note_sequence(port_name, note_sequence):
    ...
    def listen_and_continue(self, input_port, output_port):
        with open_input(input_port) as in_port, open_output(output_port) as dummy:
            ...
            while True:                                             # Infinite listening loop
                ...
                if self.played_notes and silence_duration > _silence_threshold and not self.current_note_on_dict:
