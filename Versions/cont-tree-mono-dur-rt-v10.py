...
class PrefixTreeNode:
    ...
    def is_redundant(self):
        """Détermine si un n½ud est inutile (aucun enfant et aucune continuation)."""
        return not self.children and not self.continuations

class PrefixTreeContinuator:
    """Continuateur monophonique basé sur un arbre préfixé avec suppression des branches inutiles."""
    ...
    def train(self, sequence):
        """Ajoute une séquence à l'arbre préfixé en supprimant les branches inutiles."""
        notes = [note[0] for note in sequence]  # Extraire uniquement les hauteurs
        self.sequences.append(notes)

        for start in range(len(notes) - 1):  # Construire les préfixes de droite à gauche
        ...
        self.clean_tree()  # Suppression des branches inutiles
        print(f"? Arbre mis à jour : {len(self.sequences)} séquences enregistrées.")
        self.display_memory()

    def clean_tree(self, node=None, parent=None, key=None):
        """Supprime les branches inutiles (n½uds sans continuation ni enfants)."""
        if node is None:
            node = self.root

        for note in list(node.children.keys()):
            self.clean_tree(node.children[note], node, note)

        if parent and key and node.is_redundant():
            del parent.children[key]  # Supprime les branches inutiles
    ...
    def generate(self, seed, length=10):
        """Génère une continuation monophonique avec suppression des branches inutiles."""
