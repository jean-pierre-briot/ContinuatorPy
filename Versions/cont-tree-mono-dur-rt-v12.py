...
class PrefixTreeContinuator:
    ...
    def train(self, sequence):
        """
        Ajoute une séquence à l'arbre préfixé en ne conservant que les sous-séquences des débuts.
        Ex : ABCD -> [A], [B, A], [C, B, A]
        """
        notes = [note[0] for note in sequence]  
        self.sequences.append(notes)

        # On génère uniquement les sous-séquences des débuts de la séquence
        for start in range(len(notes) - 1, 0, -1):  # On s'arrête avant la première note
            current_node = self.root
            # On prend uniquement les premiers éléments de la séquence
            ...
            # Ajouter la continuation correspondante
            if notes[start] not in current_node.continuations:
                current_node.continuations.append(notes[start])
